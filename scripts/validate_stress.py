...
def main():
    ...
    while True:
        ...
        active = db.execute_read(
            "SELECT COUNT(*) as count FROM jobs WHERE status IN ('analyzing', 'converting', 'post_processing', 'moving')"
        )
        ...
        print(
            f"Elapsed: {elapsed:.1f}s | Queued: {q_count} | Active: {a_count} | Completed: {c_count} | Duplicates: {d_count} | Failed: {f_count} | RAM: {ram:.1f}MB"
        )
    ...
    with open(csv_file, "w", encoding="utf-8") as f:
        ...
        for ts in time_series:
            f.write(
                f"{ts['elapsed']},{ts['queued']},{ts['active']},{ts['completed']},{ts['duplicates']},{ts['failed']},{ts['ram_mb']}\n"
            )

    # Save log evidence
    with open(log_file, "w", encoding="utf-8") as f:
        f.write("=== Ingestion Stress Test Report ===\n")
        f.write("Total files dropped: 100\n")
