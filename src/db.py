...
class DatabaseManager:
    ...
    def update_job_status(
        self, job_id: int, status: str, error_message: str | None = None, reason: str | None = None
    ) -> None:
        """
        ...
        if status == "converting":
            self.execute_write(
                "UPDATE jobs SET status = ?, started_at = datetime('now') WHERE id = ?", (status, job_id)
            )
            ...
            self.execute_write("UPDATE jobs SET status = ? WHERE id = ?", (status, job_id))
    ...
    def update_job_progress(self, job_id: int, progress: float, eta_seconds: float) -> None:
        ...
        """
        self.execute_write(
            "UPDATE jobs SET progress = ?, eta_seconds = ? WHERE id = ?", (progress, eta_seconds, job_id)
        )
    ...
    def get_next_queued_job(self) -> sqlite3.Row | None:
        ...
        """
        rows = self.execute_read("SELECT * FROM jobs WHERE status = 'queued' ORDER BY id ASC LIMIT 1")
