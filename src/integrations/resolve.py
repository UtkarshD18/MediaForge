...
def get_resolve_binary_path() -> Path | None:
    ...
    standard_paths = [Path("/opt/resolve/bin/resolve"), Path("/opt/resolve/resolve"), Path("/usr/bin/resolve")]
...
def run_resolve_diagnostics() -> dict[str, Any]:
    ...
    report = {
        ...
        "status_message": "DaVinci Resolve detected."
        if installed
        else "DaVinci Resolve installation directory (/opt/resolve) was not found.",
    }
