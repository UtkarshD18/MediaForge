    ...
    def process_file(self, filepath_str: str) -> None:
        ...
        try:
            ...
            active = self.db.execute_read(
                "SELECT id FROM jobs WHERE filepath = ? AND status NOT IN ('completed', 'failed')", (str(path),)
            )
            if active:
                self.logger.debug(
                    f"File {path.name} is already active in queue (Job {active[0]['id']}). Skipping duplicate."
                )
                return
